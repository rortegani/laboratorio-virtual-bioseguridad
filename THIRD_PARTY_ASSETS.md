# Third-party 3D assets

Se incorporó un asset glTF de mobiliario ambiental. No se convirtió a GLB.

Modern Wooden Cabinet se utilizó para validar el pipeline de assets, pero
actualmente no se carga ni se utiliza en la escena. Su atribución y licencia
se conservan para referencia.

## Laboratory table (pending)

- Author: yuitop
- Source: https://sketchfab.com/3d-models/laboratory-table-e1321eb1e321486f8862d52bcacc2b36
- License: CC BY
- File: `public/models/furniture/laboratory-table.glb` (no incorporado)
- Modifications: ninguna; requiere descarga/verificación manual antes de incluirlo.

## Modern Wooden Cabinet (incorporado)

- Author: Patrik Pangerl
- Source: https://polyhaven.com/a/modern_wooden_cabinet
- License: CC0
- Resolution: 1K
- Format: glTF
- File: `public/models/furniture/modern_wooden_cabinet/modern_wooden_cabinet_1k.gltf`
- Binary: `public/models/furniture/modern_wooden_cabinet/modern_wooden_cabinet.bin`
- Textures: `textures/modern_wooden_cabinet_nor_gl_1k.jpg`, `textures/modern_wooden_cabinet_arm_1k.jpg`, `textures/modern_wooden_cabinet_diff_1k.jpg`
- Use: auxiliary reception furniture in Zone 2; it does not replace `cabinetRoot`.
- Modifications: ninguna
- Fecha de incorporación: 2026-09-08

## Monitor (pending)

- Author: SweatyIce
- Source: https://opengameart.org/content/3d-model-of-a-computer-monitor-and-desk
- License: CC0
- File: `public/models/equipment/monitor.glb` (no incorporado)
- Modifications: ninguna; la fuente ofrece `Monitor.zip` en formato `.max`, no GLB/glTF.

`MANUAL_DOWNLOAD_REQUIRED`: no convertir ni incluir el ZIP actual. Solo colocar
un monitor exportado a GLB/glTF en la ruta indicada después de una conversión
externa verificable.

## Otros elementos pendientes

`trash-bin.glb` no tiene todavía una fuente compatible y visualmente adecuada
para un laboratorio clínico. No se descargó ni se incluyó.

## Descarga manual pendiente

Los siguientes assets siguen pendientes; no se descargaron ni se incluyeron:

- `public/models/furniture/laboratory-table.glb`
- `public/models/equipment/monitor.glb`
- `public/models/furniture/trash-bin.glb`
