...
export class Mission3Manager {
  ...
  acknowledgeError(): boolean { if (!this.canStart()) return false; this.state.update({ crossContaminationAcknowledged: true }); return true; }
  ...
  complete(): Mission3Result {
    ...
    const safe = !current.crossContaminationDetected;
    if (!safe && !current.crossContaminationAcknowledged) return { success: false, reason: 'contamination_not_acknowledged' };
