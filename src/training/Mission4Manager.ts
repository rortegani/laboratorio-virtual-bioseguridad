...
export type MolecularResult = { correct: true; reason: 'valid_interpretation' | 'invalid_interpretation' } | { correct: false; reason: 'mission3_incomplete' | 'area_not_reviewed' | 'analysis_not_started' | 'analysis_not_completed' | 'result_not_reviewed' | 'wrong_interpretation' };
...
export class Mission4Manager {
  ...
  reviewResult(): boolean { if (!this.state.snapshot.molecularAnalysisCompleted) return false; this.state.update({ molecularResultReviewed: true }); return true; }
  interpret(answer: MolecularAnswer): MolecularResult {
    ...
    if (!correct) return { correct: false, reason: 'wrong_interpretation' };
