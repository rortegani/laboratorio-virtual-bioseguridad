...
export class Mission5Manager {
  ...
  stopActivity(): boolean { if (!this.canStart() || !this.state.snapshot.incidentAcknowledged) return false; this.state.update({ unsafeActivityStopped: true }); return true; }
  reportIncident(): boolean { const current = this.state.snapshot; if (!this.canStart() || !current.incidentAcknowledged || !current.unsafeActivityStopped) return false; this.state.update({ incidentReported: true }); return true; }
